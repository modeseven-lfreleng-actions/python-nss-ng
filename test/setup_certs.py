...
def run_cmd(cmd_args: List[str], input: str | None = None) -> Tuple[str, str]:
    ...
    try:
        ...
        try:
            ...
            result = sp.run(
                ["pkg-config", "--variable=libdir", "nss"],
                capture_output=True,
                text=True,
                check=False,
            )
        ...
        if returncode != 0:
            raise CmdError(cmd_args, returncode, "failed %s" % (" ".join(cmd_args)), stdout, stderr)
...
def db_has_cert(options, nickname):
    cmd_args = [find_nss_tool("certutil"), "-d", options.db_name, "-L", "-n", nickname]
...
def format_cert(options, nickname):
    cmd_args = [
        find_nss_tool("certutil"),
        "-L",  # OPERATION: list
        "-d",
        options.db_name,  # NSS database
        "-f",
        options.passwd_filename,  # database password in file
        "-n",
        nickname,  # nickname of cert to list
    ]
...
def create_ca_cert(options):
    ...
    logging.info(
        'creating ca cert: subject="%s", nickname="%s"', options.ca_subject, options.ca_nickname
    )
    ...
    cmd_args = [
        ...
        options.passwd_filename,  # database password in file
        "-n",
        options.ca_nickname,  # nickname of cert being created
        "-s",
        options.ca_subject,  # subject of cert being created
        "-g",
        str(options.key_size),  # keysize
        "-t",
        "CT,,CT",  # trust
        ...
        "-v",
        str(options.valid_months),  # validity in months
        "-z",
        options.noise_filename,  # noise file random seed
    ]
...
def create_server_cert(options):
    ...
    logging.info(
        'creating server cert: subject="%s", nickname="%s"',
        options.server_subject,
        options.server_nickname,
    )
    ...
    cmd_args = [
        ...
        options.passwd_filename,  # database password in file
        "-c",
        options.ca_nickname,  # nickname of CA used to sign this cert
        "-n",
        options.server_nickname,  # nickname of cert being created
        "-s",
        options.server_subject,  # subject of cert being created
        "-g",
        str(options.key_size),  # keysize
        "-t",
        "u,u,u",  # trust
...
def create_client_cert(options):
    ...
    logging.info(
        'creating client cert: subject="%s", nickname="%s"',
        options.client_subject,
    ...
    cmd_args = [
        ...
        options.ca_nickname,  # nickname of CA used to sign this cert
        "-n",
        options.client_nickname,  # nickname of cert being created
        "-s",
        options.client_subject,  # subject of cert being created
        "-g",
        str(options.key_size),  # keysize
        "-t",
        "u,u,u",  # trust
...
def add_trusted_certs(options):
    ...
    try:
        ...
        logging.warning(
            "Could not add trusted certs module: %s", e.stderr if e.stderr else e.message
        )
...
def set_fips_mode(options):
    ...
    try:
        ...
        if (options.fips and e.returncode == FIPS_ALREADY_ON_ERR) or (
            not options.fips and e.returncode == FIPS_ALREADY_OFF_ERR
        ):
            pass
        else:
            raise
...
def setup_certs(args):
    ...
    parser = argparse.ArgumentParser(
        description="create certs for testing",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )

    parser.add_argument("--verbose", action="store_true", help="provide info level messages")

    parser.add_argument("--debug", action="store_true", help="provide debug level messages")

    parser.add_argument("--quiet", action="store_true", help="do not display any messages")

    parser.add_argument("--show-certs", action="store_true", help="show the certificate details")

    parser.add_argument(
        "--no-clean", action="store_false", dest="clean", help="do not remove existing db_dir"
    )

    parser.add_argument(
        "--no-trusted-certs",
        dest="add_trusted_certs",
        action="store_false",
    ...
    parser.add_argument("--ca-subject", help="CA certificate subject")

    parser.add_argument("--ca-nickname", help="CA certificate nickname")
    ...
    parser.add_argument("--client-username", help="client user name, used in client cert subject")
    ...
    parser.add_argument("--serial-number", type=int, help="starting serial number for certificates")

    parser.add_argument(
        "--valid-months", dest="valid_months", type=int, help="validity period in months"
    )
    parser.add_argument(
        "--path-len", dest="ca_path_len", type=int, help="basic constraints path length"
    )
    parser.add_argument("--key-type", dest="key_type", help="key type, either rsa or dsa")
    parser.add_argument(
        "--key-size", dest="key_size", help="number of bits in key (must be multiple of 8)"
    )
    parser.add_argument(
        "--serial-file", dest="serial_file", help="name of file used to track next serial number"
    )

    parser.add_argument("--db-fips", action="store_true", help="enable FIPS mode on NSS Database")
    ...
    logging.info(
        'server nickname="%s", server subject="%s"', options.server_nickname, options.server_subject
    )
    logging.info(
        'client nickname="%s", client subject="%s"', options.client_nickname, options.client_subject
    )
